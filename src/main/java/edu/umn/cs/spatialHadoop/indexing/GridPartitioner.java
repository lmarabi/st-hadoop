 ...
 * multiple grids, it replicates it to all overlapping partitions.
 * 
 * @author Ahmed Eldawy
 *
 */
public class GridPartitioner extends Partitioner {
	...
	public void createFromPoints(Rectangle mbr, Point[] points, int capacity)
		...
		do {
			...
			this.numRows = gridInfo.rows = (int) Math.ceil(numTiles
					/ gridInfo.columns);
			...
			// numTiles = (int) (numTiles * Math.ceil((double)maxCellSize /
			// capacity));
			// }
		} while (maxCellSize > capacity && maxIterations-- > 0);
		LOG.info("Partitioning the space into a " + gridInfo.columns + "x"
				+ gridInfo.rows + " grid");
	...
	public CellInfo getPartition(int partitionID) {
		...
		return new CellInfo(partitionID, x + col * tileWidth, y + row
				* tileHeight, x + (col + 1) * tileWidth, y + (row + 1)
				* tileHeight);
	}
