...
public class HilbertCurvePartitioner extends Partitioner {
	...
	protected void createFromHValues(final int[] hValues, int capacity) {
		...
		for (int i = 0; i < splits.length; i++) {
			...
			this.splits[i] = quantile == hValues.length ? maxH
					: hValues[quantile];
		}
	}
	...
	public static int computeHValue(Rectangle inMBR, final double x,
			final double y) {
	...
	public static void main(String[] args) throws IOException {
		...
		hcp.createFromPoints(inMBR, points.toArray(new Point[points.size()]),
				10);
