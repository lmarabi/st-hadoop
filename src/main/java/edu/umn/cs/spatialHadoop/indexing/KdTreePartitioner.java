...
public class KdTreePartitioner extends Partitioner {
	...
	/**
	 * Location of all splits stored in a complete binary tree which is encoded
	 * in a single array in a heap-like structure.
	 */
	...
	public void createFromPoints(Rectangle mbr, Point[] points, int capacity) {
		...
		// Enumerate all partition IDs to be able to count leaf nodes in any
		// split
		...
		// Keep splitting the space into halves until we reach the desired
		// number of
		// partitions
		...
		while (!splitTasks.isEmpty()) {
			...
			if (splitTask.partitionID < numSplits) {
				String child1 = Integer
						.toBinaryString(splitTask.partitionID * 2);
				String child2 = Integer
						.toBinaryString(splitTask.partitionID * 2 + 1);
				...
				int splitIndex = (int) (((long) size_child1 * splitTask.toIndex + (long) size_child2
						* splitTask.fromIndex) / (size_child1 + size_child2));
				partialQuickSort(points, splitTask.fromIndex,
						splitTask.toIndex, splitIndex,
						comparators[splitTask.direction]);
				Point splitValue = points[splitIndex];
				this.splits[splitTask.partitionID] = splitTask.direction == 0 ? splitValue.x
						: splitValue.y;
				splitTasks.add(new SplitTask(splitTask.fromIndex, splitIndex,
						1 - splitTask.direction, splitTask.partitionID * 2));
				splitTasks
						.add(new SplitTask(splitIndex, splitTask.toIndex,
								1 - splitTask.direction,
	...
	public void write(DataOutput out) throws IOException {
		...
		if (bbuffer.hasRemaining())
			throw new RuntimeException(
					"Did not calculate buffer size correctly");
			...
			if (splitToTest.splitID >= splits.length) {
				...
				if (splitToTest.direction == 0) {
					...
					if (shapeMBR.x1 < splits[splitToTest.splitID])
						splitsToTest.add(new SplitToTest(
								splitToTest.splitID * 2,
								1 ^ splitToTest.direction)); // Go left
					if (shapeMBR.x2 > splits[splitToTest.splitID])
						splitsToTest.add(new SplitToTest(
								splitToTest.splitID * 2 + 1,
								1 ^ splitToTest.direction)); // Go right
				} else {
	...
	public static void main(String[] args) throws IOException {
		...
		long length = inPath.getFileSystem(params).getFileStatus(inPath)
				.getLen();
