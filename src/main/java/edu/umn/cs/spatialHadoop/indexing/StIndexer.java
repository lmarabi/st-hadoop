...
public class StIndexer {
  private static final Log LOG = LogFactory.getLog(StIndexer.class);
  ...
  public static class PartitionerMap extends
    Mapper<Interval, Iterable<? extends StShape>, IntWritable, StShape> {
    ...
    @Override
    protected void map(Interval key, Iterable<? extends StShape> StShapes,
        final Context context) throws IOException,
        InterruptedException {
      final IntWritable partitionID = new IntWritable();
      for (final StShape StShape : StShapes) {
        if (replicate) {
          partitioner.overlapPartitions(StShape, new ResultCollector<Integer>() {
            @Override
            public void collect(Integer r) {
              partitionID.set(r);
              try {
                context.write(partitionID, StShape);
              } catch (IOException e) {
          ...
          partitionID.set(partitioner.overlapPartition(StShape));
  ...
  public static class PartitionerReduce<S extends StShape>
    extends Reducer<IntWritable, StShape, IntWritable, StShape> {

    @Override
    protected void reduce(IntWritable partitionID, Iterable<StShape> StShapes,
        Context context) throws IOException, InterruptedException {
      LOG.info("Working on partition #"+partitionID);
      for (StShape StShape : StShapes) {
        context.write(partitionID, StShape);
        context.progress();
      }
  ...
  private static Job indexMapReduce(Path inPath, Path outPath,
    ...
    job.setJarByClass(StIndexer.class);
    
    // Set input file MBR if not already set
    Interval inputMBR = (Interval) OperationsParams.getShape(conf, "mbr");
    if (inputMBR == null) {
    ...
    StShape shape = OperationsParams.getStShape(conf, "shape");
  ...
  public static Partitioner createPartitioner(Path[] ins, Path out,
    ...
    try {
      ...
      long t1 = System.currentTimeMillis();
      final Interval inMBR = (Interval) OperationsParams.getStShape(job, "mbr");
      ...
      if (job.get("StShape") != null)
      params2.set("StShape", job.get("StShape"));
      if (job.get("local") != null)
      params2.set("local", job.get("local"));
      params2.setClass("outStShape", Point.class, StShape.class);
      ...
      partitioner.createFromPoints_StShape(inMBR, sample.toArray(new Point[sample.size()]), partitionCapacity);
  ...
  private static void indexLocal(Path inPath, final Path outPath,
    ...
    final SpatialInputFormat3<Interval, StShape> inputFormat = new SpatialInputFormat3<Interval, StShape>();
    ...
    // Set input file MBR if not already set
    Interval inputMBR = (Interval) OperationsParams.getStShape(conf, "mbr");
    if (inputMBR == null) {
      inputMBR = FileMBR.fileMBR(inPath, new OperationsParams(conf));
      OperationsParams.setStShape(conf, "mbr", inputMBR);
    }
    ...
    final IndexRecordWriter<StShape> recordWriter = new IndexRecordWriter<StShape>(
        partitioner, replicate, sindex, outPath, conf);
    for (FileSplit fsplit : fsplits) {
      RecordReader<Interval, Iterable<StShape>> reader = inputFormat.createRecordReader(fsplit, null);
      ...
      while (reader.nextKeyValue()) {
        Iterable<StShape> StShapes = reader.getCurrentValue();
        if (replicate) {
          for (final StShape s : StShapes) {
  ...
  protected static void printUsage() {
    ...
    System.out.println("Shape:<point|Rectangle|polygon|Temporal> - (*) Type of StShapes stored in input file");
