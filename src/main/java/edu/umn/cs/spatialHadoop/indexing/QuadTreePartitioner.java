...
public class QuadTreePartitioner extends Partitioner {
	...
	// TODO instead of storing leaf node IDs, store them in a bit array to
	// ensure
	...
	public void createFromPoints(Rectangle mbr, Point[] points, int capacity) {
		...
		for (int i = 0; i < points.length; i++)
			zValues[i] = ZCurvePartitioner.computeZ(mbr, points[i].x,
					points[i].y);
	...
	protected void createFromZValues(final long[] zValues, int capacity) {
		...
		class QuadTreeNode {
			...
			public QuadTreeNode(int fromIndex, int toIndex, long minZ,
					long maxZ, int nodeID, int depth) {
		...
		QuadTreeNode root = new QuadTreeNode(0, zValues.length, minZ, maxZ, 1,
				1);
	...
	public int overlapPartition(Shape shape) {
		...
		while (nodeToSearch < leafNodes.size() && !leafNodes.get(nodeToSearch)) {
			...
			} else if (queryPoint.x < nodeCenter.x
					&& queryPoint.y >= nodeCenter.y) {
			...
			if (shapeMBR.isIntersected(nodeToSearch)) {
				if (leafNodes.get(nodeToSearch.cellId)) {
					...
					nodesToSearch.add(new CellInfo(nodeToSearch.cellId * 4,
							nodeToSearch.x1, nodeToSearch.y1, centerPoint.x,
							centerPoint.y));
					nodesToSearch.add(new CellInfo(nodeToSearch.cellId * 4 + 1,
							nodeToSearch.x1, centerPoint.y, centerPoint.x,
							nodeToSearch.y2));
					nodesToSearch.add(new CellInfo(nodeToSearch.cellId * 4 + 2,
							centerPoint.x, nodeToSearch.y1, nodeToSearch.x2,
							centerPoint.y));
					nodesToSearch.add(new CellInfo(nodeToSearch.cellId * 4 + 3,
							centerPoint.x, centerPoint.y, nodeToSearch.x2,
							nodeToSearch.y2));
				}
			}
		}
	...
	public CellInfo getPartition(int partitionID) {
		...
		int partitionDepth = (KdTreePartitioner
				.getNumberOfSignificantBits(partitionID) + 1) / 2;
