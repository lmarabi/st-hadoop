...
public class ZCurvePartitioner extends Partitioner {
	...
	protected void createFromZValues(final long[] zValues, int capacity) {
		...
		for (int i = 0; i < numSplits; i++) {
			...
			this.zSplits[i] = quantile == zValues.length ? maxZ
					: zValues[quantile];
		}
	}
	 ...
	 * 
	 * @param x
	 *            - integer value of the x-axis (cannot exceed
	 *            Integer.MAX_VALUE)
	 * @param y
	 *            - integer value of the y-axis (cannot exceed
	 *            Integer.MAX_VALUE)
	 * @return
	 */
...
=======
	...
	public static java.awt.Point unComputeZOrder(long morton,
	...
	public static java.awt.Rectangle getMBRInteger(long zMin, long zMax) {
		...
		java.awt.Point minXY = unComputeZOrder(zMin & (~mask),
				new java.awt.Point());
		java.awt.Point maxXY = unComputeZOrder(zMin | mask,
				new java.awt.Point());
		java.awt.Rectangle mbr = new java.awt.Rectangle(minXY.x, minXY.y,
	...
	public static Rectangle getMBR(Rectangle mbr, long zMin, long zMax) {
		...
		trueMBR.x1 = (double) (mbrInteger.x) * mbr.getWidth() / Resolution
				+ mbr.x1;
		trueMBR.y1 = (double) (mbrInteger.y) * mbr.getHeight() / Resolution
				+ mbr.y1;
		trueMBR.x2 = (double) (mbrInteger.getMaxX()) * mbr.getWidth()
				/ Resolution + mbr.x1;
		trueMBR.y2 = (double) (mbrInteger.getMaxY()) * mbr.getHeight()
				/ Resolution + mbr.y1;
		return trueMBR;
	}
