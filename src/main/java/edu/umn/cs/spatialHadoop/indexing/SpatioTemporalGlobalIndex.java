 ...
 * @param <S>
 */
public class SpatioTemporalGlobalIndex<S extends StShape> implements Writable, Iterable<S> {
  ...
  public SpatioTemporalGlobalIndex() {
  }
  ...
  public void readFields(DataInput in) throws IOException {
    ...
    this.shapes = (S[]) new StShape[length];
  ...
  public int rangeQuery(StShape queryRange, ResultCollector<S> output) {
    int result_count = 0;
    for (S shape : shapes) {
  ...
  public static<S1 extends StShape, S2 extends StShape>
      int spatialJoin(SpatioTemporalGlobalIndex<S1> s1, SpatioTemporalGlobalIndex<S2> s2,
  ...
  public Interval getMBR() {
    ...
    if (!i.hasNext())
      return null;
    Interval globalMBR = new Interval(Long.MIN_VALUE,Long.MAX_VALUE);
    while (i.hasNext()) {
  ...
  public int knn(final double qx, final double qy, int k, ResultCollector2<S, Double> output) {
    ...
    do {
      ...
      Interval queryRange = new Interval();
