...
public class STRPartitioner extends Partitioner {
	...
	public void createFromPoints(Rectangle mbr, Point[] points, int capacity) {
		...
		for (int column = 0; column < columns; column++) {
			...
			// Determine the x split for this column. Last column has a special
			// handling
			this.xSplits[column] = col_quantile == points.length ? mbr.x2
					: points[col_quantile - 1].x;
			// 2- Partition this column vertically in the same way
			Arrays.sort(points, prev_quantile, col_quantile,
			...
			for (int row = 0; row < rows; row++) {
				int row_quantile = (prev_quantile * (rows - (row + 1)) + col_quantile
						* (row + 1))
						/ rows;
				// Determine y split for this row. Last row has a special
				// handling
				this.ySplits[column * rows + row] = row_quantile == col_quantile ? mbr.y2
						: points[row_quantile].y;
			}
	...
	public void write(DataOutput out) throws IOException {
		...
		ByteBuffer bbuffer = ByteBuffer
				.allocate((xSplits.length + ySplits.length) * 8);
	...
	public void overlapPartitions(Shape shape, ResultCollector<Integer> matcher) {
		...
		for (int col = col1; col <= col2; col++) {
			...
			int cell1 = Arrays.binarySearch(ySplits, col * rows, (col + 1)
					* rows, shapeMBR.y1);
			if (cell1 < 0)
				cell1 = -cell1 - 1;
			int cell2 = Arrays.binarySearch(ySplits, col * rows, (col + 1)
					* rows, shapeMBR.y2);
			if (cell2 < 0)
				cell2 = -cell2 - 1;
	...
	public int overlapPartition(Shape shape) {
		...
		if (col < 0)
			col = -col - 1;
		int cell = Arrays.binarySearch(ySplits, col * rows, (col + 1) * rows,
				center.y);
		if (cell < 0)
			cell = -cell - 1;
		return cell;
