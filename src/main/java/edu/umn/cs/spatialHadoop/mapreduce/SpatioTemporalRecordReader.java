 ...
 *
 */
public class SpatioTemporalRecordReader<V extends StShape> extends
    RecordReader<Partition, Iterable<V>> {
  
  private static final Log LOG = LogFactory.getLog(SpatioTemporalRecordReader.class);
  ...
  public void initialize(InputSplit split, Configuration conf)
    ...
    this.stockShape = (V) OperationsParams.getStShape(conf, "shape");
  ...
  protected boolean isMatched(StShape shape) {
   ...
   * @author Eldawy
   */
  public static class ShapeIterator<V extends StShape>
      implements Iterator<V>, Iterable<V> {
    protected V shape;
    protected V nextShape;
    private SpatioTemporalRecordReader<V> srr;
    
    public ShapeIterator() {
    }

    public void setSpatialRecordReader(SpatioTemporalRecordReader<V> srr) {
      this.srr = srr;
      try {
        if (shape != null)
