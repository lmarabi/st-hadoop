		...
		do {
			...
			if ((inObj instanceof STPoint)) {
				...
				while (it.hasNext()) {
					Map.Entry entry = (Map.Entry) it.next();
					String id = (String) entry.getKey();
					STNycTrajectory traj_rect = (STNycTrajectory) entry.getValue();
					DTW similarityfunc = new DTW(query, traj_rect.getPointsList());
					pqueue.add(new Score(id, similarityfunc.getDistance()));
				}
			}
		...
		if (pqueue.size() >= topk) {
			...
			for (int i = 0; i < topk; i++) {
				...
				if ((inObj instanceof STPoint)) {
					...
					STNycTrajectory trajectory = overlappedTrajectory_rect.get(temp.id);
